...
import 'package:musan_client/src/ui/auth/thank_you_for_reg.dart';
...
class LoginProvider extends ChangeNotifier {
    ...
    googleSignIn.onCurrentUserChanged.listen((GoogleSignInAccount account) async {
      try {
        ...
        if (account != null) {
          ...
          String body =
              '{"mobile": "${currentUser.id}","password": "123456789",  "deviceToken": null}';
  ...
  saveLogin(LoginReponse loginReponse) {
    SharedPreferences.getInstance().then((value) async {
      ...
      await value
          .setString(Finals.USER_EMAIL, loginReponse.result.user.email)
          .then((value) => null);
      await value.setString(Finals.USER_TOKEN, loginReponse.result.token).then((value) => null);
      await value
          .setString(Finals.USER_REFRESH_TOKEN, loginReponse.result.refreshToken)
          .then((value) => null);
      await value
          .setString(Finals.USER_ID, loginReponse.result.user.id.toString())
          .then((value) => null);
      await value.setString(Finals.USER_NAME, loginReponse.result.user.name).then((value) => null);
      await value
          .setString(Finals.USER_TYPE_ID, loginReponse.result.user.userTypeId.toString())
          .then((value) => null);
      await value
          .setString(Finals.USER_PHONE, loginReponse.result.user.phoneNumber ?? "")
          .then((value) => null);
      await value
          .setString(Finals.USER_COUPON, loginReponse.result.user.couponCode ?? "")
          .then((value) => null);
      userMobileNumber = loginReponse.result.user.phoneNumber;

      Get.to(() => ThankYouForReg());
