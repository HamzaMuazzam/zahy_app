...
class DashboardProvider extends ChangeNotifier {
  ...
  initAnalytics(){
    ...
    void setUserProfileData(GetUserProfile userProfileFromJson, bool isUserProfileLoaded) {
    this.userProfileFromJson = userProfileFromJson;
    this.isUserProfileLoaded = isUserProfileLoaded;

    editProfileNumberController.text=userProfileFromJson.result.phoneNumber.toString();
    ...
    final GlobalKey<ScaffoldState> dashboardBoardScaffoldKey =new GlobalKey<ScaffoldState>();
  ...
  Future inviteScreenOnInviteTapped() {
    if (inviteFormKey.currentState.validate()) {
      if (inviteScreenInviteSelection == null) {
        ...
        launch('sms:${inviteMobileNumberController.text}?body=Hello, this is Musan we are lucky to have a chance to invited you by our customer, please check the link below to download the app… enjoy 😉 \n Musan.net');
  ...
  void sub_unsub_topic(bool settingValue) {
   ...
   Firebase.initializeApp().then((value) async {
    ...
    if(settingValue){
      await FirebaseMessaging.instance.unsubscribeFromTopic(shared.getString(Finals.USER_ID));
      await FirebaseMessaging.instance.unsubscribeFromTopic(shared.getString(Finals.USER_TYPE_ID));
    ...
    {
      await FirebaseMessaging.instance.subscribeToTopic(shared.getString(Finals.USER_ID));
      await FirebaseMessaging.instance.subscribeToTopic(shared.getString(Finals.USER_TYPE_ID));
  ...
  void setPaymentHistory(bool isPaymentHistoryLoaded, PaymentHistoryResponse paymentHistoryResponseFromJson) {
  ...
  void setCarPickOrderID(String carPickupOrderId, String carPickUpId, String workshopID) {
  ...
  hitWorkshopOrTechnician(bool isWorkShop) async {
    if (isWorkShop) {
      await collectionReference.collection("userTypes").doc('workshop').set({'WhatUpdated': "Workshop_NewOfferReceived_${DateTime.now().toString()}"});
    }
    else if (!isWorkShop) {
      // await ref.child("/userTypes/technician").set({'WhatUpdated': "Technician_NewOfferReceived_${DateTime.now().toString()}"});
      await collectionReference.collection("userTypes").doc('technician').set({'WhatUpdated': "Technician_NewOfferReceived_${DateTime.now().toString()}"});
    }
  }

  hitAllCarPickups(String workshopId, String orderId) async {
    await collectionReference.collection('users').doc(workshopId??"").set({"WhatUpdated":"OrderProgressed","OrderId":orderId});

    await collectionReference.collection("userTypes").doc('carpickup').set({'WhatUpdated': "CarPickUp_NewOfferReceived_${DateTime.now().toString()}"});
  }

  void hitWorkshopForOfferNegotiation(String offerid, String price, String workshopId) async {
    await collectionReference.collection('users').doc(workshopId??"").set({"WhatUpdated":"OfferNegotiationFromClient","OfferId":offerid,
      "ClientId":userID});
  }

  void hitSpecificWorkshop(String workshopId, String orderId) async {
    await collectionReference.collection("userTypes").doc('workshop').set({'WhatUpdated': "Workshop_NewOfferReceived_${DateTime.now().toString()}"});
    await collectionReference.collection('users').doc(workshopId??"").set({"WhatUpdated":"OfferAccepted", "ClientId":userID,"OrderId":orderId});
  }
  void hitSpecificWorkshopForOfferRejected(String workshopId) async {
    await collectionReference.collection('users').doc(workshopId??"demo").set({"WhatUpdated":"OfferRejected","workshopId":workshopId});
  }

  void hitWorkshopForOrderProgressed(workshopId,orderId,{String data:"OrderProgressed"})async {
    collectionReference.collection("users").doc(workshopId.toString()).set({"WhatUpdated":data,"OrderId":orderId.toString()});
  }

  void hitCarPickUpForFInalPaymenDone(String carPickIDOrderID, String orderId,String carpickId)async {
    await collectionReference.collection('users').doc(carpickId).set({"WhatUpdated":"OrderProgressed"});


  }


  Future startPayingPayment(String paymentType,String paymentAmount,) async {
    await Get.to(()=>PaymentWebView(url:'https://mucp.deeps.info/Home/Payment?userId=$userID&orderId='
        '$orderId&amount=${(double.parse(paymentAmount)*100).toInt()}')).then((value){
      print(value);
      if(value!=null){
        ...
        if(value){
          ...
          if(paymentType=="OrderPart".tr){
            ApiServices.acceptPart(isPaid:true,partID:partID,orderId:orderId,workshopId: workshopId);
            ApiServices.hitNotificationAfterPaymentMade(orderId.toString());

          }
          else if (paymentType=="CarPickUp".tr){
            ApiServices.payCarPickUpFinalBill(dashboardProvider.carPickupOrderId,dashboardProvider.carPickUpId);
            ApiServices.hitNotificationAfterPaymentMade(carPickupOrderId.toString());
          }
  ...
  void setCashback(bool isCashBackLoaded, PromotionCashBack promotionCashBackFromJson) {
  ...
  factory RealTimeSingleton(FirebaseFirestore collectionReference)  {
    if (_instance == null) {
      ...
      collectionReference.collection("clients").doc(dashboardProvider.userID).snapshots().listen((event) async {
        ...
        else if(event.data().toString().contains("OfferReNegotiatedFromWorkShop")){
...
_listenOrderProgressed(Map<String, dynamic> data){
  ...
  if(Get.currentRoute.contains("OrderTracking") && orderScreenProvider.singalOrderId == orderId.toString()){

    if(data.toString().contains("OrderFinished")){

      while(Get.isDialogOpen){Get.back();}

      while(Get.isBottomSheetOpen){Get.back();}
